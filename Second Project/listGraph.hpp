...
private:
    ...
    std::vector<std::vector<std::pair<int, int>>> adj_list; // (indeks s¹siada, waga)
public:
...
template<typename Vertex>
void List_Graph<Vertex>::addEdge(const Vertex& u, const Vertex& v, int weight) {
    int iu = idx.at(u);
    int iv = idx.at(v);
...
template<typename Vertex>
std::vector<std::pair<Vertex, int>> List_Graph<Vertex>::neighbours(const Vertex& u) const {
    int iu = idx.at(u);
...
template<typename Vertex>
int List_Graph<Vertex>::getWeight(const Vertex& u, const Vertex& v) const {
    int iu = idx.at(u);
    int iv = idx.at(v);
    for (const auto& p : adj_list[iu]) {
        if (p.first == iv) return p.second;
    }
    return std::numeric_limits<int>::infinity(); // brak krawêdzi
}