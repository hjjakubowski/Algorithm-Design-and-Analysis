...
template<typename Vertex>
void Matrix_Graph<Vertex>::addEdge(const Vertex& u, const Vertex& v) {
    int iu = idx.at(u);
    int iv = idx.at(v);
    adj_matrix[iu][iv] = adj_matrix[iv][iu] = true;
}

template<typename Vertex>
std::vector<Vertex> Matrix_Graph<Vertex>::neighbours(const Vertex& u) const {
    int iu = idx.at(u);
